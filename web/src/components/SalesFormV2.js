...
class SalesFormV2 extends Component {
	...
	addClick() {
		...
		this.setState(prevState => {
			const updateFilteredProducts = prevState.filteredProducts.push(this.state.products);
		});
	}
		...
		for (let i = 0; i < this.state.count; i++) {
            ...
            <hr/>
					...
					<div className="form-group col-2">
						...
						>
							<i className="fa fa-times-circle-o big pointer" id="trash" />
						</button>
					</div>
					...
					<div className="form-group col">
                        ...
                        className="text-center amount-input"
							...
							<div
								...
								onClick={() => {
									const updatedUnitAmount = this.state.unitAmount.slice();
									updatedUnitAmount[i] = Number(updatedUnitAmount[i]) + Number(1);
									if (this.state.productID[i]) {
										//check if stock is ok
										const correspondingProduct = this.state.products.filter(product => {
											return this.state.productID[i] === product._id;
										...
										if (
											Number(correspondingProduct.stock) < Number(updatedUnitAmount[i])
										) {
											// stock validation failed
											alert(
												`Amount can not exceed ${correspondingProduct.title}'s stock (${
	...
	render() {
		const { products, customers, startDate, newCustomer } = this.state;
				...
				{customers &&
					products && (
                     ...
                     <hr/>
										...
										{customers.map(m => {
											return (
												<option key={m._id} name={m._id} value={m._id}>
													{m.phone} - {m.firstName === "" ? "(unknown)" : m.firstName}{" "}
							...
							<input
								type="submit"
								className="btn btn-primary btn-lg col mt-4"
								value="Create Sale"
							/>
						</form>
					)}
